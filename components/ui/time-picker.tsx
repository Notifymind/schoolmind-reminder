  ...
  return (
    <fieldset disabled={disabled} className="min-w-0 space-y-4 disabled:opacity-50">
          ...
          {(["hours", "minutes"] as const).map((item, index) => (
            <React.Fragment key={item}>
              ...
              <div className="space-y-1 text-center">
                {isTyping ? (
                  <Input
                    aria-label={item === "hours" ? "Hour (24-hour)" : "Minute"}
                    type="number"
                    inputMode="numeric"
                    min={0}
                    max={item === "hours" ? 23 : 59}
                    value={item === "hours" ? pad(hours) : pad(minutes)}
                    onChange={(event) => {
                      if (!event.target.value) return;
                      const next = Number(event.target.value);
                      if (!Number.isInteger(next) || next < 0 || next > (item === "hours" ? 23 : 59)) return;
                      onChange(item === "hours" ? `${pad(next)}:${pad(minutes)}` : `${pad(hours)}:${pad(next)}`);
                    }}
                    onFocus={(event) => event.target.select()}
                    className="h-16 w-20 rounded-xl text-center text-4xl tabular-nums md:text-4xl"
                  />
                ) : (
                  <button
                    type="button"
      ...
      <div className="flex items-center gap-2">
        <Button type="button" variant="ghost" size="icon" aria-label={isTyping ? "Switch to clock" : "Type time"} onClick={() => setIsTyping(!isTyping)}>
          {isTyping ? <Clock className="size-4" /> : <Keyboard className="size-4" />}
        </Button>
        <p id={hintId} className="text-xs text-muted-foreground">
          {isTyping ? "Enter a time in 24-hour format." : `Select ${part}. Use arrow keys to adjust.`}
        </p>
      </div>
    </fieldset>
  );
